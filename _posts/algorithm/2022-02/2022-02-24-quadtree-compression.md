...
result 리스트에서 0번째 인덱스에는 0의 갯수, 1번째 인덱스에는 1의 갯수 이므로 해당 좌표의 값을 인덱스로 해서 갯수를 하나씩 세면 깔끔한 코딩이 가능하다.
...
좌표 범위 안에 있는 n개의 사각형과 0의 갯수 혹은 1의 갯수가 완벽하게 똑같으면 압축이 가능하다.
...
분할한 사분면별로 다시 체크해서 0과 1의 갯수를 세온다. 이 때 마법의 함수 f()를 사용한다. (`*area`는 x1, y1, x2, y2 타이핑 다시 하기 귀찮아서 저렇게 그대로 넣어줬다.)
